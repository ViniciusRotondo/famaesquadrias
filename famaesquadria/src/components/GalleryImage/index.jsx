...
const GalleryImage = ({ images }) => {
    ...
    return(
        <div className="gap-10 p-20 shadow-sm grid" style={{gridTemplateColumns:"repeat(auto-fill, minmax(250px, 2fr)"}}>
            {images.map((image) => (
            ...
            {selectedImage && (
             <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-80 flex items-center justify-center" onClick={closeModal}>
                <img className="w-2/5 h-4/5" src={selectedImage.url} alt={selectedImage.title}></img>
